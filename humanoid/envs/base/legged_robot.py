...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.init_done = True
        # self.traj = torch.zeros((273, 12), device=self.device, dtype=torch.float)
        self.traj = torch.zeros((273, 60), device=self.device, dtype=torch.float)
        ...
        self.motor_direction = torch.tensor(motor_direction, device=self.device, dtype=torch.float)
    ...
    def step(self, actions):
        ...
        # self.actions[:, 0:self.num_control] = torch.clip(actions, -clip_actions, clip_actions).to(self.device)
        ...
        for _ in range(self.cfg.control.decimation):
            ...
            if self.cfg.domain_rand.randomize_obs_motor_lag_timesteps:
                q = (self.dof_pos - self.default_dof_pos) * self.obs_scales.dof_pos
                dq = self.dof_vel * self.obs_scales.dof_vel
                self.obs_motor_lag_buffer[:,:,1:] = self.obs_motor_lag_buffer[:,:,:self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]].clone()
                self.obs_motor_lag_buffer[:,:,0] = torch.cat((q, dq), 1).clone()
            if self.cfg.domain_rand.randomize_obs_imu_lag_timesteps:
                self.gym.refresh_actor_root_state_tensor(self.sim)
                # print("The current linear velocity is: ", quat_rotate_inverse(self.base_quat, self.root_states[:, 7:10]))
                self.base_quat[:] = self.root_states[:, 3:7]
                self.base_ang_vel[:] = quat_rotate_inverse(self.base_quat, self.root_states[:, 10:13])
                self.base_euler_xyz = get_euler_xyz_tensor(self.base_quat)
                self.obs_imu_lag_buffer[:,:,1:] = self.obs_imu_lag_buffer[:,:,:self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]].clone()
                self.obs_imu_lag_buffer[:,:,0] = torch.cat((self.base_ang_vel * self.obs_scales.ang_vel, self.base_euler_xyz * self.obs_scales.quat), 1).clone()
    ...
    def reset(self):
        ...
        obs, privileged_obs, _, _, _ = self.step(torch.zeros(
            ...
            # self.num_envs, self.num_control, device=self.device, requires_grad=False))
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.domain_rand.add_obs_lag:
            self.obs_motor_lag_buffer[env_ids, :, :] = 0.0
            self.obs_actions_lag_buffer[env_ids, :, :] = 0.0
            self.obs_imu_lag_buffer[env_ids, :, :] = 0.0
            if self.cfg.domain_rand.randomize_obs_motor_lag_timesteps:
                self.obs_motor_lag_timestep[env_ids] = torch.randint(self.cfg.domain_rand.obs_motor_lag_timesteps_range[0],
                                                        self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]+1, (len(env_ids),),device=self.device)
                if self.cfg.domain_rand.randomize_obs_motor_lag_timesteps_perstep:
                    self.last_obs_motor_lag_timestep[env_ids] = self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]
            else:
                self.obs_motor_lag_timestep[env_ids] = self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]

            if self.cfg.domain_rand.randomize_obs_actions_lag_timesteps:
                self.obs_actions_lag_timestep[env_ids] = torch.randint(self.cfg.domain_rand.obs_actions_lag_timesteps_range[0],
                                                        self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]+1, (len(env_ids),),device=self.device)
                if self.cfg.domain_rand.randomize_obs_actions_lag_timesteps_perstep:
                    self.last_obs_actions_lag_timestep[env_ids] = self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]
            else:
                self.obs_actions_lag_timestep[env_ids] = self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]

            if self.cfg.domain_rand.randomize_obs_imu_lag_timesteps:
                self.obs_imu_lag_timestep[env_ids] = torch.randint(self.cfg.domain_rand.obs_imu_lag_timesteps_range[0],
                                                        self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]+1, (len(env_ids),),device=self.device)
                if self.cfg.domain_rand.randomize_obs_imu_lag_timesteps_perstep:
                    self.last_obs_imu_lag_timestep[env_ids] = self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]
            else:
                self.obs_imu_lag_timestep[env_ids] = self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]
    ...
    def _process_rigid_body_props(self, props, env_id):
        ...
        if self.cfg.domain_rand.randomize_com:
             props[0].com = gymapi.Vec3(self.com_displacements[env_id, 0] + self.cfg.domain_rand.com_offset_x, self.com_displacements[env_id, 1] + self.cfg.domain_rand.com_offset_y,
    ...
    def _compute_torques(self, actions):
        ...
        if self.cfg.domain_rand.randomize_coulomb_friction:
            ...
            torch_rand_float(self.cfg.domain_rand.joint_viscous_range[0], self.cfg.domain_rand.joint_viscous_range[1], (self.num_envs, self.num_actions), device=self.device) * self.dof_vel -\
            torch_rand_float(self.cfg.domain_rand.joint_coulomb_range[0], self.cfg.domain_rand.joint_coulomb_range[1], (self.num_envs, self.num_actions), device=self.device)
         
        else: 
            torques = p_gains * \
                (self.lagged_actions_scaled + self.default_dof_pos -
                self.dof_pos + self.motor_offsets) - d_gains * self.dof_vel
    ...
    def _init_buffers(self):
        ...
        self.base_ang_vel = quat_rotate_inverse(self.base_quat, self.root_states[:, 10:13])
        self.projected_gravity = quat_rotate_inverse(self.base_quat, self.gravity_vec)
        self.z_contact_force_current = torch.zeros(self.num_envs, len(self.feet_indices), dtype=torch.float32, device=self.device, requires_grad=False)
        if self.cfg.terrain.measure_heights:
        ...
        if self.cfg.domain_rand.add_obs_lag:
            self.obs_motor_lag_buffer = torch.zeros(self.num_envs,self.num_actions * 2,self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]+1,device=self.device)
            self.obs_actions_lag_buffer = torch.zeros(self.num_envs,self.num_actions,self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]+1,device=self.device)
            self.obs_imu_lag_buffer = torch.zeros(self.num_envs, 6, self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]+1,device=self.device)

            if self.cfg.domain_rand.randomize_obs_motor_lag_timesteps:
                self.obs_motor_lag_timestep = torch.randint(self.cfg.domain_rand.obs_motor_lag_timesteps_range[0],
                                                        self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]+1, (self.num_envs,),device=self.device)
                if self.cfg.domain_rand.randomize_obs_motor_lag_timesteps_perstep:
                    self.last_obs_motor_lag_timestep = torch.ones(self.num_envs,device=self.device,dtype=int) * self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]
            else:
                self.obs_motor_lag_timestep = torch.ones(self.num_envs,device=self.device) * self.cfg.domain_rand.obs_motor_lag_timesteps_range[1]

            if self.cfg.domain_rand.randomize_obs_actions_lag_timesteps:
                self.obs_actions_lag_timestep = torch.randint(self.cfg.domain_rand.obs_actions_lag_timesteps_range[0],
                                                        self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]+1, (self.num_envs,),device=self.device)
                if self.cfg.domain_rand.randomize_obs_actions_lag_timesteps_perstep:
                    self.last_obs_actions_lag_timestep = torch.ones(self.num_envs,device=self.device,dtype=int) * self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]
            else:
                self.obs_actions_lag_timestep = torch.ones(self.num_envs,device=self.device) * self.cfg.domain_rand.obs_actions_lag_timesteps_range[1]

            if self.cfg.domain_rand.randomize_obs_imu_lag_timesteps:
                self.obs_imu_lag_timestep = torch.randint(self.cfg.domain_rand.obs_imu_lag_timesteps_range[0],
                                                        self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]+1, (self.num_envs,),device=self.device)
                if self.cfg.domain_rand.randomize_obs_imu_lag_timesteps_perstep:
                    self.last_obs_imu_lag_timestep = torch.ones(self.num_envs,device=self.device,dtype=int) * self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]
            else:
                self.obs_imu_lag_timestep = torch.ones(self.num_envs,device=self.device) * self.cfg.domain_rand.obs_imu_lag_timesteps_range[1]
    ...
    def _create_envs(self):
        ...
        print("This number ", self.num_bodies, " should be 19, and this number ", self.num_dofs, " should be 18")
