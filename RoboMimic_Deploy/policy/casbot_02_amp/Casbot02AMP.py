"""
Casbot02AMP — AMP locomotion policy for the Casbot 02 7DOF (29-DOF) robot.

Standalone policy class. Ported from CasbotAMP (25-DOF), adapted for the
29-joint casbot_02_7dof robot trained with the MJLAB AMP framework.

Observation layout (96 dims per frame, 4-frame history = 384 total):
...
Joint order (matches XML and MJLAB training order):
    ...
    Left arm (15-21): shoulder_pitch, shoulder_roll, shoulder_yaw, elbow_pitch,
                       wrist_yaw, wrist_pitch, wrist_roll
    Right arm (22-28):shoulder_pitch, shoulder_roll, shoulder_yaw, elbow_pitch,
                       wrist_yaw, wrist_pitch, wrist_roll
"""
...
class Casbot02AMP:
    """AMP locomotion policy for the Casbot 02 7DOF robot."""

    def __init__(self, config_path=None):
        current_dir = os.path.dirname(os.path.abspath(__file__))

        if config_path is None:
            config_path = os.path.join(current_dir, "config", "Casbot02AMP.yaml")
        ...
        print(f"[Casbot02AMP] Policy initialized.")
    ...
    def init_buffers(self, base_quat, ang_vel, q, dq):
        """Fill the observation history buffer."""
        self._vCmdBodyPast = np.zeros(3, dtype=np.float32)
        self._last_action = np.zeros(self.num_actions, dtype=np.float32)
        self.obs_buffer = np.zeros(self.num_obs, dtype=np.float32)
        ...
        print(f"[Casbot02AMP] Buffers initialized ({self.history_length} frames)")
    ...
    def step(self, base_quat, ang_vel, cmd_vel, q, dq):
        ...
        if terminated:
            print(
                f"[Casbot02AMP WARNING] Large anchor proj_gravity error: "
    ...
    def _observations_compute(self, base_quat, ang_vel, cmd_vel, q, dq):
        ...
        # Build current robot state (96 dims)
    ...
    def _action_compute(self, observation):
        ...
        try:
            ...
            print(f"[Casbot02AMP ERROR] ONNX inference failed: {e}")
            return {
