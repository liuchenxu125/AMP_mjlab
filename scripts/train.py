...
def run_train(task_id: str, cfg: TrainConfig, log_dir: Path) -> None:
  ...
  # Dump before the runner injects runtime objects such as
  # ``symmetry_cfg["_env"]`` (the live env, which contains an unpickleable
  # ``mujoco.MjSpec``). Rank 0 only, to avoid multi-GPU races.
  if rank == 0:
