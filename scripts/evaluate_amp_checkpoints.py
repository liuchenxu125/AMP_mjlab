"""Headless, repeatable checkpoint comparison for CASBOT02 leg AMP locomotion."""
...
def main() -> None:
    ...
    for index, checkpoint in enumerate(checkpoints, start=1):
      ...
      with torch.no_grad():
        for step in range(args.steps):
          ...
          standing = torch.linalg.vector_norm(command, dim=1) < 0.2
          moving = ~standing
          ...
          if step >= args.warmup_steps:
            ...
            _append_mean(samples["landing_mean"], landing_values)
            ...
            _append_mean(samples["moving_slip_mean"], moving_slip)
            ...
            q = robot.data.joint_pos[:, leg_ids]
            ...
            if standing.any():
              projected_gravity = robot.data.projected_gravity_b[standing]
              ...
              height = robot.data.root_link_pos_w[standing, 2]
              _append_mean(samples["stand_height"], torch.abs(height - desired_height))

              foot_pos = robot.data.site_pos_w[standing][:, foot_ids, :2]
              delta = foot_pos[:, 0] - foot_pos[:, 1]
              ...
              _append_mean(samples["stand_angvel"], torch.linalg.vector_norm(
                robot.data.root_link_ang_vel_b[standing, :2], dim=-1
              ))
      ...
      row: dict[str, float | str | int] = {
        ...
        "landing_force_p95_n": _percentile(samples["landing_p95"], 95),
        "moving_slip_mean_mps": _mean(samples["moving_slip_mean"]),
        "moving_slip_p95_mps": _percentile(samples["moving_slip_p95"], 95),
  ...
  json_path.write_text(json.dumps({
    ...
    "waist_com_x_range": env_cfg.events["waist_com_backward"].params["ranges"][0],
