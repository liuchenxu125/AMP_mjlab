...
def effort_limits_with_delayed_actuators(
    env: ManagerBasedRlEnv,
    env_ids: torch.Tensor | None,
    ...
    """Randomize effort limits, unwrapping delayed actuators when needed."""
    asset: Entity = env.scene[asset_cfg.name]
    ...
    for actuator in actuators:
        ...
        if isinstance(actuator, (BuiltinPositionActuator, XmlPositionActuator)):
            ...
            raise TypeError(
                "effort_limits_with_delayed_actuators only supports "
                "BuiltinPositionActuator, XmlPositionActuator, and IdealPdActuator "
                f"optionally wrapped with DelayedActuator, got {type(actuator).__name__}"
            )
