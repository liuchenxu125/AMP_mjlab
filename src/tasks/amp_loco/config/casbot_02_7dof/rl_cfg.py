"""RL configuration for Casbot 02 7DOF AMP locomotion task."""
...
class RslRlAmpRunnerCfg(RslRlOnPolicyRunnerCfg):
  ...
  min_normalized_std: List[float] = field(default_factory=lambda: [0.05] * 27)
  amp_body_names: tuple = ()
  amp_anchor_name: str = ""


def casbot_02_7dof_amp_ppo_runner_cfg() -> RslRlAmpRunnerCfg:
  """Create RL runner configuration for Casbot 02 7DOF AMP locomotion task."""
  return RslRlAmpRunnerCfg(
    ...
    experiment_name="casbot_02_7dof_amp_locomotion",
    logger="tensorboard",
    save_interval=1000,
    ...
    min_normalized_std=[0.05] * 29,  # 29 DOF (incl. head)
    amp_body_names=(
      "base_link",
    ...
    amp_anchor_name="waist_yaw_link",
  )
