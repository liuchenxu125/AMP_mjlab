"""RL configuration for Casbot Skeleton AMP locomotion task."""
...
_MOTION_DATA_DIR = os.path.join(
  ...
  "src", "assets", "motions", "casbot_skeledon", "amp",
)
...
class RslRlAmpRunnerCfg(RslRlOnPolicyRunnerCfg):
  ...
  min_normalized_std: List[float] = field(default_factory=lambda: [0.05] * 25)
  amp_body_names: tuple = ()
  amp_anchor_name: str = ""


def casbot_amp_ppo_runner_cfg() -> RslRlAmpRunnerCfg:
  """Create RL runner configuration for Casbot Skeleton AMP locomotion task."""
  return RslRlAmpRunnerCfg(
    ...
    experiment_name="casbot_skeleton_amp_locomotion",
    logger="tensorboard",
    save_interval=100,
