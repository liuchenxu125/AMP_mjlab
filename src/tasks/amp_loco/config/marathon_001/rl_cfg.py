"""RL configuration for Marathon_001 AMP locomotion task.

Marathon_001: 18 DOF humanoid (12 leg + 6 arm), no waist/head/wrist.
"""
...
_MOTION_DATA_DIR = os.path.join(
  ...
  "src", "assets", "motions", "marathon_001", "amp",
)
...
class RslRlAmpRunnerCfg(RslRlOnPolicyRunnerCfg):
  ...
  min_normalized_std: List[float] = field(default_factory=lambda: [0.05] * 18)
  amp_body_names: tuple = ()
  amp_anchor_name: str = ""


def marathon_amp_ppo_runner_cfg() -> RslRlAmpRunnerCfg:
  """Create RL runner configuration for Marathon_001 AMP locomotion task."""
  return RslRlAmpRunnerCfg(
    ...
    experiment_name="marathon_001_amp_locomotion",
    logger="tensorboard",
    save_interval=100,
