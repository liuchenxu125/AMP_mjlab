"""Marathon_001 AMP Locomotion environment configurations.

Marathon_001: 18 DOF humanoid with 6-DOF arms (shoulder_pitch/roll, elbow_pitch ×2).
Root body: "base_link" — arms and legs attach directly to base_link.
No separate torso, no waist, no head, no wrist joints.
"""
...
def marathon_amp_rough_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Marathon_001 rough terrain AMP configuration."""
  ...
  cfg.scene.entities = {"robot": get_marathon_robot_cfg()}
  ...
  joint_pos_action.scale = MARATHON_ACTION_SCALE
  ...
  _motion_base = os.path.join(
    os.path.dirname(__file__), "..", "..", "..", "..", "assets", "motions", "marathon_001", "amp"
  )
...
def marathon_amp_flat_env_cfg(play: bool = False) -> ManagerBasedRlEnvCfg:
  """Create Marathon_001 flat terrain AMP configuration."""
  cfg = marathon_amp_rough_env_cfg(play=play)
